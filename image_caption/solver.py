...
class CaptioningSolver(object):
    ...
    def train(self):
        ...
        captions_data = self.data['captions']
        image_idxs_data = self.data['image_idxs']
        ...
        with tf.Session(config=config) as sess:
            ...
            for e in range(self.n_epochs):
                ...
                for i in range(n_iters_per_epoch):
                    ...
                    feed_dict = {self.model.feature: features_batch, self.model.word:word, self.model.target:target}
                    ...
                    if (i+1) % self.print_every == 0:
                        ...
                        saver.save(sess, os.path.join(self.model_path, 'model'), global_step=gs)
                ...
                if self.print_bleu:
                    ...
                    print "bleu: val shape:",val_features.shape
                    ...
                    print "all decode shape:",len(all_decoded), all_decoded[0]
                    save_pickle(all_decoded, "%s/val/val.candidate.captions.pkl"%path)
                    scores = evaluate(data_path='%s/'%path, split='val', get_scores=True)
    ...
    def test(self, data, split='train', attention_visualization=True, save_sampled_captions=True):
        ...
        with tf.Session(config=config) as sess:
            ...
            if save_sampled_captions:
                ...
                save_pickle(all_decoded, "%s/%s/%s.candidate.captions.pkl" %(path, split,split))
